...
// SonVHc defined
#define BkavLight1 BSP_IO_PORT_04_PIN_10
#define BkavLight2 BSP_IO_PORT_04_PIN_11
...
// SonVHc-Bkav add
        //Trigger Word ID
      if ((150 <= nMapID && nMapID < 159)){
          s_nCommandRecognizeLimit = COMMAND_STAGE_TIME_MIN_Bkav; //reset time out//
      }
      //Turn on the light ID
      else if(130 <= nMapID && nMapID < 140 ){
          R_IOPORT_PinWrite(&g_ioport_ctrl, BkavLight1, BSP_IO_LEVEL_HIGH);
          R_IOPORT_PinWrite(&g_ioport_ctrl, BkavLight2, BSP_IO_LEVEL_HIGH);
          DBG_UART_TRACE("BkavLight: Turn on the light.\r\n\r\n\r\n");
          s_nCommandRecordSample = 1000000000;          // timeout
      }

      // Turn off the light ID
      else if(140 <= nMapID && nMapID <= 149){
          R_IOPORT_PinWrite(&g_ioport_ctrl, BkavLight1, BSP_IO_LEVEL_LOW);
          R_IOPORT_PinWrite(&g_ioport_ctrl, BkavLight2, BSP_IO_LEVEL_LOW);
          DBG_UART_TRACE("BkavLight: Turn off the light.\r\n\r\n\r\n");
          s_nCommandRecordSample = 1000000000;          // timeout
      }

      //Response Voice--SonVHc
      if (!PlaySpeexMapID(pbySpeexDataBegin, nMapID))
          DBG_UART_TRACE("Fail to play Speex by MapID(%d).\r\n\r\n\r\n", nMapID);
