...
1. Copy the packed model file(XXX_packed_withTxt.bin) to src/Model.
...
1. #define RECOG_FLOW               RECOG_FLOW_TWO_STAGE
...
2. #define RECOG_FLOW               RECOG_FLOW_ONE_STAGE
   We recognize only one group model at a time. Please choice the recognizing group model
   by define:
   #define ONE_STAGE_GROUP_INDEX       GROUP_INDEX_COMMAND
...
6. #define SUPPORT_VAD
   We can use VAD to saving power for product using battery. We switch to normal operation 
   frequency when VAD detect the voice, and switch to low frequency when VAD detect the end 
   of voice.
