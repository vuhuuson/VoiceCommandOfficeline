 ...
 *   nSampleRate(IN): It could be 8000, 16000.
 ...
 *                    For 8 KHz narrow-band, the supported SpeexHeader.nBitRate are:
 *                       4000(very poor), 6000(poor), 8000(acceptable), 11200(acceptable), 15200(acceptable)
 *                    For 16 KHz wide-band, the supported SpeexHeader.nBitRate are:
 *                       8000(poor), 10000(bad), 12800(good), 16800(better), 20800(good enough), 24000(very good)
 *                    The SpeexHeader.nBitRate is not same as real bit rate, please refer SpeexApiConst.h.
 ...
 *   nInFrameSamples(IN): The sample count of lpsInFrame, it must be equal to SpeexHeader.nFrameSamples.
 *   lpbyOutEncode(OUT): The output of encoded data.
 *   nOutEncodeSize(IN): The byte count of lpbyOutEncode, it must be equal to SpeexHeader.nEncodeSize.
 * Returns 0 on success, negative on error.
 */
