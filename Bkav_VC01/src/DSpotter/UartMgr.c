...
const uart_instance_t *g_pUart = NULL;
int g_nUartChannel = -1;

uint32_t g_nUartWriteCount = 0;
volatile bool g_bUartDataWriting = false;
volatile bool g_bUartDataReading = false;
volatile bool g_bUartClosing = true;
PfnOnDataReadCompleteCallback g_pfnOnDataReadCompleteCallback = NULL;
...
int UartOpen(const uart_instance_t *pUart, int nUartChannel, int nBaudRate)
{
    ...
    err = pUart->p_api->versionGet(&uart_version);
    if (err != FSP_SUCCESS)
    {
        DBGTRACE("pUart->p_api->versionGet() err = %d!\n", err);
        return err;
    }
    DBGTRACE("UART API version is: %02d.%02d\n", (int16_t) uart_version.api_version_major,
              (int16_t) uart_version.api_version_minor);
    DBGTRACE("UART code version is: %02d.%02d\n", (int16_t) uart_version.code_version_major,
              (int16_t) uart_version.code_version_minor);
